...
import { Users, Trophy, ArrowUpRight, Plus, Copy, Delete, HelpCircle, Check, ArrowLeft, LogOut, Play, RotateCcw, CalendarDays, Send, UserPlus } from "lucide-react";
...
export default function Game() {
    ...
    async function load(r = current.current.room) { try {
        const res = await fetch(`/api/game${r ? '?room=' + encodeURIComponent(r) : ''}`);
 ...
 <div className="board-status" aria-live="polite">{finished ? <><strong>{game.status === 'won' ? 'Beautifully played.' : 'That was a tricky one.'}</strong> The word was <b>{game.answer}</b>. <button onClick={() => copy(`Wordmates ${room ? 'Round ' + data.room.round : data.date} ${game.status === 'won' ? rows.length : 'X'}/6\n${rows.map(r => r.marks.map(m => ['⬛', '🟨', '🟦'][m]).join('')).join('\n')}\n${location.origin}${room ? '/?room=' + room : ''}`)}>Share result <ArrowUpRight size={14}/></button></> : !data?.user && !loading ? <a href={signin} target="_top">Sign in with ChatGPT to make your first guess <ArrowUpRight size={15}/></a> : loading ? 'Setting up your board…' : busy ? 'Checking your word…' : rows.length === 0 ? 'Your opening word is a blank canvas.' : `${6 - rows.length} guesses left. You’ve got this.`}</div>
 ...
 {dialog === 'play' ? <div><button className="button full" disabled={busy} onClick={async () => { if (!data?.user) {
    ...
    catch { } }}><label htmlFor="name">Display name</label><input id="name" value={name} onChange={e => setName(e.target.value)} maxLength={24} required/><button className="button full" disabled={busy}><Check size={16}/>Save name</button><a className="text-button back-link" href="/signout-with-chatgpt?return_to=/" target="_top"><LogOut size={16}/>Sign out</a></form>}</>}
